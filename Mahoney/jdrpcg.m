...
if SHOW && size(history,1)>0
   ...
   date=fix(clock);
   String=sprintf('\n %2 i-%2 i-%2 i, %2 i:%2 i:%2 i',date(3:-1:1),date(4:6));

     t=history(:,3);
     plot(histjd(:,2),log10(histjd(:,1)),'b*',...
          t,log10(history(:,4)),'b-',t,log10(tol)+0*t,'k:' )
     legend('log_{10} || r_{# MV} ||_ 2 at JD steps','log_{10} || r_{# MV} ||_ 2 during PCG'), 
     title(StringT)
   ...
   if k>0, 
    ...
    Str=[sprintf('\n Detected eigenvalues:     Corresponding residual norm:%15 s',''),Str];
    fprintf('% s \n',Str)
   else, fprintf('\n'); 
   end
