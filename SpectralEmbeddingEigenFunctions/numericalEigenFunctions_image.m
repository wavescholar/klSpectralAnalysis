function [bins,g,lambdas,pp]=numericalEigenFunctions_image(DATA,SIGMA,EPSILON,NUM_BINS,figs)
  %
  ...
  % for image: don't use histogram, just use all data points!
 ...
 % [pp,bins]=hist(DATA,nBins);
  ...
  SIGMA = round(median(sqrt(D(:))))
  W=exp(-0.5*D.^2/SIGMA^2); % W is \tilde{W} in eqn.2.
  if figs
    figure; imagesc(W); title('Wsquig','FontSize',20);
  end
  ...
  hatD=diag(sum(P*W,1));
  if figs
    figure; imagesc(hatD); title('hatD','FontSize',20);
  end
  ...
  if figs
    figure; imagesc(PWP); title('P*Wsquiq*P','FontSize',20);
    figure; imagesc(tildeD); title('tildeD','FontSize',20);
  end
