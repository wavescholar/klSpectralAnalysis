...
% 
% INPUT: 
%   X: D-by-N data matrix with the rows being the points
...
%                            Default: false.
%       [sparsifying_method]:'ksvd' (by M. Elad et al.) or 'spams' (by J. Mairal)
%                            Default: 'ksvd'.
...
%                            Default: false. 
%       [threshold2]       : threshold for determining the intersection dimension of the wavelet subspaces 
%                            associated to all children of a fixed node.
%                            Default: 0.1.
%       [mergePsiCapIntoPhi]:whether to merge the common part of the wavelet subspaces 
%                            associated to the children into the scaling function of the parent.
%                            Default: false. 
%       [GWTversion]       : one of the following versions:
...
% 
% Publications:
%   1. Multiscale Geometric Methods for Data Sets II: Geometric Wavelets, W.K. Allard, G. Chen and M. Maggioni, to appear in ACHA, 2011 
...
if ~isfield(opts, 'GWTversion') || isempty(opts.GWTversion),   opts.GWTversion = 1;        end;
...
if ~isfield(opts, 'knnAutotune'),          opts.knnAutotune = 20;                end;
if ~isfield(opts, 'smallestMetisNet'),     opts.smallestMetisNet = 10;           end;
...
if (opts.splitting || opts.pruning) && ~isfield(opts, 'threshold2'),          opts.threshold2 = 1e-1;            end;
if opts.splitting && ~isfield(opts, 'mergePsiCapIntoPhi'),  opts.mergePsiCapIntoPhi = false;   end;
...
if isempty(opts.graph),
    ...
    opts.graph = GraphDiffusion( X, 0, struct('KNN', opts.knn,'kNNAutotune',opts.knnAutotune,'Display',0,'kEigenVecs',50,'Symmetrization','W+Wt','NNMaxDim',100) );
    gMRA.Timing.graph = cputime-gMRA.Timing.graph;
    gMRA.Graph = opts.graph;
...
if isempty(opts.tree) || isempty(opts.treeidxs),
    ...
    [~,gMRA.cp,cmember]   = nesdis(opts.graph.W,'sym',opts.smallestMetisNet);
