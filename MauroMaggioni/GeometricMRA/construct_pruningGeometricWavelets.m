function gW = construct_pruningGeometricWavelets( gW )
...
gW.dictEncodingCosts = zeros(1,nAllNets);  
...
while j>0 && n>0
    ...
    for i = 1:length(nodes)
        ...
        if numel(children)>1 % multiple children, construct local geometric wavelets and compare among the different encoding costs
            ...
            cost_p = gW.dictEncodingCosts(node); % parent only, computed in local_SVD_analysis above
            cost_c = sum(gW.dictEncodingCosts(children)); % children only, already optimized in previous loop                     
            ...
            switch bestMethod
                ...
                case 3 % parent+wavelets is the best 
                    ...
                    gW.dictEncodingCosts(node) = minCost;
                    ...
                    for c = 1:length(children)
                        ...
                        if ~gW.opts.sparsifying
                            gW.WavConsts{children(c)} = gW.WavConsts{children(c)} - gW.ScalFuns{node}*(gW.ScalFuns{node}'*gW.WavConsts{children(c)});
                        else
                            gW.WavConsts{children(c)} = gW.WavConsts{children(c)} - gW.ScalFuns{node}*(gW.ScalFuns{node}\gW.WavConsts{children(c)});
                        end
...
if gW.opts.addTangentialCorrections
    ...
    cost_w = sum(gW.dictEncodingCosts(children)) + intersectionDimension*(gW.Sizes(node)+gW.opts.AmbientDimension);
else
    ...
    cost_w = sum(gW.dictEncodingCosts(children)) + intersectionDimension*(gW.Sizes(node)+gW.opts.AmbientDimension) + gW.opts.AmbientDimension*nChildren;
end
...
for c = 1:nChildren
    ...
    if gW.opts.addTangentialCorrections
        ...
        cost_w = cost_w + size(gW.WavBases{children(c)},2)*gW.opts.AmbientDimension;
    else
        %cost_w = cost_w - (size(gW.ScalFuns{children(c)},2)-size(gW.WavBases{children(c)},2))*(gW.Sizes(children(c))+gW.opts.AmbientDimension);
        cost_w = cost_w + (size(gW.WavBases{children(c)},2)-size(gW.ScalFuns{children(c)},2))*(gW.opts.AmbientDimension);
    end
...
for parentDimension = 1:size(gW.ScalFuns{node},2)-1
    ...
    if gW.opts.addTangentialCorrections
        ...
        cost_w = sum(gW.dictEncodingCosts(children)) + (intersectionDimension+parentDimension)*(gW.opts.AmbientDimension);
    else
        ...
        cost_w = sum(gW.dictEncodingCosts(children)) + (intersectionDimension+parentDimension)*(gW.opts.AmbientDimension) + gW.opts.AmbientDimension*nChildren;
    end
    ...
    for c = 1:nChildren
        ...
        if gW.opts.addTangentialCorrections
            ...
            cost_w = cost_w + size(tempWavBases{c},2)*gW.opts.AmbientDimension;
        else
            %cost_w = cost_w - (size(gW.ScalFuns{children(c)},2)-size(tempWavBases{c},2))*(gW.Sizes(children(c))+gW.opts.AmbientDimension);
            cost_w = cost_w - (size(gW.ScalFuns{children(c)},2)-size(tempWavBases{c},2))*gW.opts.AmbientDimension;
        end
