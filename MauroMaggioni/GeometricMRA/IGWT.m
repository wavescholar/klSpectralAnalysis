...
x_mat = (reshape([gW.WavConsts{Data.chain}], [], j_max))'; % matrix of recovered coordinates at all scales, initialized as the wavelet constant at each scale

for j = j_max:-1:1
    
    if ~isempty(Data.WavCoeffs{j})
        x_mat(j,:) = x_mat(j,:) + Data.WavCoeffs{j}*gW.WavBases{Data.chain(j)}';
    end
...
if gW.opts.addTangentialCorrections
    ...
    for j = j_max-1:-1:2
        x_mat(j,:) = x_mat(j,:) - sum(x_mat(j+1:j_max,:),1)*gW.ScalFuns{Data.chain(j-1)}*gW.ScalFuns{Data.chain(j-1)}';
    end
    
end

x_mat = cumsum(x_mat,1);
x = x_mat(end,:);
