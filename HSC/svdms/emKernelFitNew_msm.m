function [st,own,K,R,Ar]  = emKernelFitNew_msm(u0,K,flag)
...
%% EM expression: u0' * log(K * st) + lambda * (sum(st) - 1)
 ...
 % tol = 0.01;
  ...
  if (flag)
    ...
    %subplot(2,1,1)
    %plot(st,'co-'); 
    plot(st,'mo-');  title('st','FontSize',18); 
    hold on
    %stem(st,'b'); 
    %subplot(2,1,2);cla;
    figure(113); %clf; 
    plot(u0,'b');    
    hold on;
    plot(K*st,'go-'); title('K * st (green), u0 (blue)','FontSize',18); 
    ax = axis;
  ...
  % commented out 11-14
    ...
    if (flag)
      ...
      plot(st1,'rx-'); title('st updated','FontSize',18); 
      %plot(st1,'bx-'); 
      ...
      hold on;
      plot(K*st1,'ro-');
      title(sprintf('blue: u0, green: K*st: itr: %d KL: %2.3f KLchange: %2.3f',itr,btsupd,changeFrac),'FontSize',18);
      hold on; plot(u0,'b'); 
...
%  colones = ones(size(K,1),1) ; % commented out vmb, bc don't seem to use
%  rowones = ones(1,size(K,2)) ; % commented out vmb, bc don't seem to use
  % get w(h|x)
  % speed this up with a for loop !!
  %Q = K.*(colones*st');

  % commented out by vmb, bc Q is reset in next set of lines:
%  Q = spalloc(size(K,1), size(K,2), size(K,2)*max(full(sum(K>0,1))));
  ...
  % trying to fix memory problems, 06-18, vmb
  R = [];
  return;
  
  % removed full, bc memory problems. hope this fixes things!
 % R  = Ar * spdiags(full(sum(Ar,1))'.^-1, 0, size(Ar,2), size(Ar,2));
 sizAr = size(Ar,2); 
 if sizAr < 20000
    R  = Ar * spdiags((sum(Ar,1))'.^-1, 0, sizAr, sizAr);
 else
     n = round(sizAr/4);
     temp = spdiags((sum(Ar,1))'.^-1, 0, sizAr, sizAr);
     R1 = Ar(1:n,:)*temp;
     R2 = Ar(n+1:2*n,:)*temp;
     R3 = Ar(2*n+1:3*n,:)*temp;
     R4 = Ar(3*n+1:end,:)*temp;
     clear temp;
     R = [R1; R2; R3; R4];
     clear R1;
     clear R2;
