...
%
  ...
  while k <= length(okIds) & ~done & (sum(pixLoc) < length(pixLoc))
    ...
    if k <= length(okIds)
      %sMp(nb,okIds(k+1:end)) = 0;
      sMp(pixLoc,okIds(k)) = 0;      
    end
  ...
  if (~skip)
    k = 1;
    while k <= length(pixMiss) & (sum(pixLoc) < length(pixLoc))
      % pick a kernel
      fprintf('%d ',k);
      id = pixMiss(k);
	...
	%sMp(nb,pixMiss(k+1:end)) = 0;
	sMp(pixLoc,pixMiss(min(k+1,length(pixMiss)))) = 0;	
      end
