function [result,msg] = eps2xxx(epsFile,outFormats,fullGsPath,orientation,resolution,dointerp)
%EPS2XXX Converts an eps file to one or several other formats using GhostScript (GS)
%
%   [result,msg] = eps2xxx(epsFile,outFormats,fullGsPath,orientation,resolution,dointerp)
%
...
%
%   - resolution:   resolution of the output file. Integer value. Default: 300 dpi.
%
%   - dointerp:     true (default) / false. Activate the dDOINTERPOLATE
%                   option of Ghostscript. Can be useful if GS interpolates
%                   an "imagesc" figure.
%   
...
%
%J.Rossel: 
%   - added an input for resolution (31.3.08)
%   - use a temporary tmp.eps file to avoid naming problems with ghostscript (3.04.08)    
%   - added an input for interpolation (29.07.08)
...
%Modif to allow resolution and dointerp as input
...
if nargin < 6 | isempty(dointerp),
    dointerp = true;
end
if dointerp,
    interpoption = '-dDOINTERPOLATE ';
else
    interpoption = '';
end
GS_PARAMETERS_PDF = ['-q -dNOPAUSE -dBATCH ',interpoption,'-dUseFlateCompression=true -sDEVICE=pdfwrite -r',num2str(resolution)];
GS_PARAMETERS_JPEG = ['-q -dNOPAUSE -dBATCH ',interpoption,'-dUseFlateCompression=true -sDEVICE=jpeg -r',num2str(resolution)];
GS_PARAMETERS_PNG = ['-q -dNOPAUSE -dBATCH ',interpoption,'-dUseFlateCompression=true -sDEVICE=png16m -r',num2str(resolution)];
GS_PARAMETERS_TIFF = ['-q -dNOPAUSE -dBATCH ',interpoption,'-dUseFlateCompression=true -sDEVICE=tiffg4 -r',num2str(resolution)];
...
%%%%%%%%%%%%%%%%
%JR modif: copy the epsfile on tmp.eps (old ghostscript doesn't like fancy
%file names)
intermedfilename = fullfile(pathstr,'tmp.eps');
copyfile(source,intermedfilename);
oldsourceName = sourceName;
sourceName = 'tmp';
source = intermedfilename;
%%%%%%%%%%%%%%%%
...
%%%%%%%%%%
%JR modif: replace tmp files by their real name
for ii = 1:nTargets,
    [tmp,tmp,ext] = fileparts(targets{ii});
    movefile(targets{ii},fullfile(pathstr,[oldsourceName,ext]));
end
%delete tmp.eps
delete(intermedfilename)
%%%%%%%%%
